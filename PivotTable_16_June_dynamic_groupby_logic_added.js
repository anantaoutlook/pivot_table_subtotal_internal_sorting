...
function PivotTable(element, props) {
  ...
  console.log('numGroups, columns ',numGroups, columns.length);
    ...
    if ($(this).find('th').length === maxTh) {
      if ($(this).find('th')[0].attributes[0]) {
        if ($(this).find('th')[0].attributes[0].name === 'rowspan') {
          ...
          const totalCols = numGroups +  columns.length;
          const sumKeys = Object.keys(addition);
          const numberOfThToBeInserted = numGroups-1;
          console.log('total cols, sum keys', totalCols, sumKeys.length);
          console.log('numberOfThToBeInserted', numberOfThToBeInserted);
          for (let thInsertIndex = 1; thInsertIndex < numberOfThToBeInserted; thInsertIndex++) {
            console.log('subTotalRow Before', $subTotalRow[0].innerHTML);
            $subTotalRow.append(`<th></th>`);
            console.log('$subTotalRow After', $subTotalRow[0].innerHTML);
          }

          sumKeys.forEach(key => {
            if (addition.hasOwnProperty(key)) {
              // console.log(formatNumber(columnFormats, addition[key]));
              $subTotalRow.append(
          ...
          // console.log('subtotal', subTotalRowData);
          allRowsWithSum.push({
            sum: addition,
