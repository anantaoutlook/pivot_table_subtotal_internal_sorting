        ...
        if ($(this).find('th')[0].attributes[0].name === 'rowspan') {
          ...
          for (
            ...
            tds.each(function (tdIndex) {
              ...
              console.log('tds[tdIndex]', $(tds[tdIndex]).attr('data-sort'));
              delete addition[tdIndex];
              addition[tdIndex] =
                parseFloat(previous) + parseFloat($(tds[tdIndex]).attr('data-sort'));
            });
          ...
          sumKeys.forEach(key => {
            if (addition.hasOwnProperty(key)) {
              ...
              $subTotalRow.append(
                `<td style="font-weight:500;" data-sort="${addition[key]}">${formatNumber(format, addition[key])}</td>`,
              );
            }
          });
