  ...
  $container.find('tbody tr').each(function eachRow(index) {
    ...
    if ($(this).find('th').length === maxTh) {
      if ($(this).find('th')[0].attributes[0]) {
        if ($($(allRows[index])).find('th[rowspan').attr('rowspan') && ($($(allRows[index + 1])).find('th[rowspan').attr('rowspan') === undefined)) {
          ...
          for (
            ...
            const tdLength = $(allRows[index + rowSpanIndex]).find('td').toArray().length;
            let sortBy = 0;
            if(tdLength) {
            ...
            if (rowSpanIndex === 0) {
              ...
              rowsArray.push({
                startIndex: index,
                value: tds[0] ? parseFloat(tds[0].innerHTML) : '',
        ...
        if ($($(allRows[index])).find('th[rowspan').attr('rowspan') && $($(allRows[index + 1])).find('th[rowspan').attr('rowspan')) {
          const rowSpanCount = parseInt(
            $(this).find('th')[0].attributes[0].value,
            10,
          );
          ...
          for (
            ...
            if (rowSpanIndex === 0) {
              ...
              let rowSpanCountOne = $($(allRows[index + rowSpanIndex])).find('th[rowspan').attr('rowspan');
              if(rowSpanCountOne) {
                rowSpanCountOne = parseInt(rowSpanCountOne, 10);
                const innerRowArray = [];
                let newTHeadOne = '';
                for (let innerRowIndex = 0; innerRowIndex < rowSpanCountOne; innerRowIndex += 1) {
                  
                  const inHtml = $(allRows[index + rowSpanIndex + innerRowIndex])[0].innerHTML;
                  const tdsOne = $($(allRows[index + rowSpanIndex + innerRowIndex])[0]).find('td');
                  let sortColValue = 0;
                  if (
                    $(tdsOne[sortBy]).attr('data-sort') !== '-Infinity' &&
                    $(tdsOne[sortBy]).attr('data-sort') !== ''
                  ) {
                    sortColValue = parseInt($(tdsOne[sortBy]).attr('data-sort'), 10);
                  }
                  if(innerRowIndex === 0) {
                    newTHeadOne = `<th rowspan="${rowSpanCountOne}">${
                      $(allRows[index + rowSpanIndex + innerRowIndex]).find('th[rowspan]')[0]
                        .innerHTML
                    }</th>`;
                    ...
                    innerRowArray.push({
                      startIndex: index,
                      value: sortColValue,
                      rowValue: `${$(allRows[index + rowSpanIndex + innerRowIndex])[0].innerHTML}`,
                    });
                  } else {
                    innerRowArray.push({
                      startIndex: index,
                ...
                const sortedArray = innerRowArray.sort((a, b) => b.value - a.value);
