...
function PivotTable(element, props) {
  ...
  // let previousIndex = 0;
  /* const indexSpanCountArray = [];
  const arr = $(allRows).toArray();
  $container.find('tbody tr').each(function eachRow(index) {
    const spanCount = $($(allRows[index])).find('th[rowspan').attr('rowspan');
    if(spanCount) {
      indexSpanCountArray.push({
        index: index,
        count: spanCount
      })
    }
  });
  console.log('indexSpanCountArray', indexSpanCountArray); */
  $container.find('tbody tr').each(function eachRow(index) {
    if (index === 0) {
      maxTh = $(this).find('th').length;
      maxTd = $(this).find('td').length;
    }
    if ($(this).find('th').length === maxTh) {
      if ($($(allRows[index])).find('th[rowspan').attr('rowspan')) {
        // previousIndex = index;
        ...
        for (
          ...
          if (rowSpanIndex === 0) {
            newTHead = `<th rowspan="${rowSpanCount + 1}">${
              $(allRows[index + rowSpanIndex]).find('th[rowspan]')[0].innerHTML
            }</th>`;
        ...
        tds.each(function (tdIndex) {
          ...
          delete sum[tdIndex];
          sum[tdIndex] =
            parseFloat(previous) + parseFloat(tds[tdIndex].innerHTML); */
        });
        allRowsWithSum.push({
          sum,
  ...
  function replaceAfterSort() {
    allRowsWithSum.pop();
    const sortedTable = allRowsWithSum.sort(keysrt(sortByColumn));
    let rows = '';
    ...
    const $lastRow = $('<tr><th>All</th></tr>');
    for (let thIndex = 0; thIndex < maxTh; thIndex += 1) {
      if (thIndex !== 0) {
        $lastRow.append('<th></th>');
      }
    }
    for (let tdIndex = 0; tdIndex < maxTd; tdIndex += 1) {
      const sum = sortedTable.reduce(
        (partialSum, a) =>
          partialSum + (a.sum ? (a.sum[tdIndex] ? a.sum[tdIndex] : 0) : 0),
        0,
      );
      const metric = cols[tdIndex];
      const format = columnFormats[metric] || numberFormat || '.3s';
      const formattedSum = formatNumber(format, sum);
      $lastRow.append(`<td>${formattedSum}</td>`);
    }
    rows += `<tr>${$lastRow[0].innerHTML}</tr>`;
