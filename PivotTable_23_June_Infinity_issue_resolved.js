...
function PivotTable(element, props) {
    ...
    if ($(this).find('th').length === maxTh) {
      if ($(this).find('th')[0].attributes[0]) {
        if ($(this).find('th')[0].attributes[0].name === 'rowspan') {
          ...
          const start = index;
          const end = index + rowSpanCount;
          const newArray = allRows.toArray();
          console.log('start, end', start, end);
          console.log('newArray', newArray);
          const subArray = newArray.slice(start, end);
          processRowSpan(subArray);
          const addition = {};
          const rowsArray = [];
          let newTHead = '';
          for (
            ...
            tds.each(function (tdIndex) {
              if($(tds[tdIndex]).attr('data-sort') !== "-Infinity" && $(tds[tdIndex]).attr('data-sort') !== ""){
  ...
  function processRowSpan(inputArray){
